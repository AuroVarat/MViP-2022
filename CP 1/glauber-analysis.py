...
spin = np.ones([N,N]) #initialise spins randomly
index_matrix = np.random.randint(0,N, size=(pnum*nstep,2)) #initialise random values
...
#%%
file_object = open('energy-glauber-{}.dat'.format(time.time()), 'a')
...
#update loop here - for Glauber dynamics
    for sweep in tqdm(range(nstep)):
        for loop in range(pnum):
            glauber_dynamic_conventional(sweep+loop*pnum)
